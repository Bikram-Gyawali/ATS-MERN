...
const forget_password = async (req, res, next) => {
  ...
  const sendVerifyEmail = async (key, name) => {
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  //Is user email is verified or not
  if (user.isVerified == false) {
    return res.status(401).json({ error: 'Email address is not activated' });
  }
  ...
  try {
    ...
    await user.save();
    sendVerifyEmail(key, user.f_name);
